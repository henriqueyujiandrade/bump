...
const Register = () => {
  ...
  const schema = yup.object().shape({
    ...
    passwordConfirm: yup.string().required("Campo Obrigatório").oneOf([yup.ref("password")], "Senhas não são iguais"),
  })
          ...
          <Stack spacing={4}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} spacing={8}>
              ...
              <FormControl py={2} >
                ...
                <InputGroup>
                  <Input id="passwordConfirm" {...register("passwordConfirm")} type={showPassword ? 'text' : 'password'} />
                  <InputRightElement h={'full'}>
                    <Button
                      variant={'ghost'}
                      onClick={() =>
            ...
            <Stack pt={4}>
              <Text align={'center'}>
                Já possui conta ? <Link to="/login" style={{color: '#8518F5'}} >Login</Link>
              </Text>
            </Stack>
          </Stack>
        </Box>
